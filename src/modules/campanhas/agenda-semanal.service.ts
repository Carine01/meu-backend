...
export class AgendaSemanalService {
  ...
  async executarAgendaDoDia(): Promise<void> {
    ...
    for (const regra of regras) {
      ...
      try {
        ...
        const leads = await this.buscarLeadsPorEtiquetas(
          regra.publicoEtiquetas,
        );

        this.logger.log(
          `${leads.length} leads encontrados para "${regra.objetivo}"`,
        );
  ...
  private async buscarLeadsPorEtiquetas(etiquetas: string[]): Promise<Lead[]> {
    try {
      ...
      const err = error as Error;
      this.logger.error(
        `Erro ao buscar leads por etiquetas: ${err.message}`,
        err.stack,
      );
      return [];
    }
