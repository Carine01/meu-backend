...
export class FilaService {
  ...
  async adicionarNaFila(
    ...
    if (!template.ativo) {
      this.logger.warn(`Adicionando mensagem inativa à fila: ${mensagemKey} para ${leadNome}`);
    }
  ...
  async cancelarMensagem(filaId: string): Promise<boolean> {
    try {
      ...
      throw new HttpException('Erro ao cancelar mensagem', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   */
  async listarPorStatus(status: FilaEnvio['status'], limit: number = 50): Promise<FilaEnvio[]> {
    try {
      const snapshot = await this.firestore
        .collection(this.COLLECTION_NAME)
        .where('status', '==', status)
      ...
      throw new HttpException('Erro ao listar fila de envio', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
