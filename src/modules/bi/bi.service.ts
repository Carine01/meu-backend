...
import {
  ...
  Agendamento,
  FilaEnvio,
} from "../mensagens/entities/mensagem.entity";
...
export class BiService {
  ...
  async getDashboardMetrics(): Promise<DashboardMetrics> {
    ...
    try {
      ...
      const scoreMedioLeads =
        totalLeads > 0 ? Math.round(somaScores / totalLeads) : 0;
      const percentualQuente =
        totalLeads > 0 ? Math.round((countQuente / totalLeads) * 100) : 0;
      const percentualMorno =
        totalLeads > 0 ? Math.round((countMorno / totalLeads) * 100) : 0;
      const percentualFrio =
        totalLeads > 0 ? Math.round((countFrio / totalLeads) * 100) : 0;
      ...
      const noShowPct =
        agendados30d > 0 ? Math.round((noShow30d / agendados30d) * 100) : 0;
      ...
      // === FILA DE ENVIO ===
      const [filaPendenteSnapshot, filaEnviados30d, filaFalhas30d] =
        await Promise.all([
          this.firestore
      ...
      this.logger.log(
        `Dashboard metrics calculadas: ${JSON.stringify(metrics)}`,
      );
      return metrics;
    } catch (error: any) {
      const err = error as Error;
      this.logger.error(
        `Erro ao buscar métricas do dashboard: ${err.message}`,
        err.stack,
      );
      throw error;
    }
...
`.trim();
  ...
  async getAnaliseFunil(): Promise<{
    ...
    const etapas = [
      ...
      },
      {
        ...
        percentual:
          metrics.leads30d > 0
            ? Math.round((metrics.agendados30d / metrics.leads30d) * 100)
            : 0,
      },
      {
        etapa: "3. Compareceu",
  ...
  async getTopEtiquetas(
    limit: number = 10,
  ): Promise<Array<{ etiqueta: string; count: number }>> {
    try {
      ...
      const err = error as Error;
      this.logger.error(
        `Erro ao buscar top etiquetas: ${err.message}`,
        err.stack,
      );
      return [];
    }
  ...
  > {
    try {
      ...
      const origemStats: Record<
        string,
        { leads: number; agendamentos: Set<string> }
      > = {};
      ...
      const err = error as Error;
      this.logger.error(
        `Erro ao calcular performance por origem: ${err.message}`,
        err.stack,
      );
      return [];
    }
