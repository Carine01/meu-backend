...
describe("WebhookService", () => {
          ...
          useValue: {
            get: jest.fn((key: string) => {
              ...
              if (key === "MAKE_WEBHOOK_URL")
                return "https://hook.us1.make.com/test";
              if (key === "MAKE_TOKEN") return "make-token-456";
              if (key === "ZAPIER_WEBHOOK_URL")
                return "https://hooks.zapier.com/test";
              return null;
            }),
          },
        },
  ...
  describe("sendWebhook", () => {
    ...
    it("deve lançar erro se WEBHOOK_URL não estiver configurado", async () => {
      ...
      const serviceWithoutUrl =
        moduleWithoutUrl.get<WebhookService>(WebhookService);

      await expect(
        serviceWithoutUrl.sendWebhook({ test: true }),
      ).rejects.toThrow("WEBHOOK_URL não configurado");
    });
  ...
  describe("sendToMake", () => {
    ...
    it("deve retornar falha se MAKE_WEBHOOK_URL não configurado", async () => {
      ...
      const serviceWithoutMake =
        moduleWithoutMake.get<WebhookService>(WebhookService);
      const result = await serviceWithoutMake.sendToMake({ test: true });

      expect(result).toEqual({
        ok: false,
        message: "Make.com não configurado",
      });
    });
  });
